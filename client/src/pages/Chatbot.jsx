...
export default function Chatbot() {
  const [messages, setMessages] = useState([
    {
      ...
      type: "bot",
      content:
        "Hello! I'm your AI financial assistant. I can help you understand your spending, budgets, debts, and provide personalized financial advice. What would you like to know?",
  ...
  const models = [
    {
      ...
      params: "1B",
      speed: "Quick",
      desc: "Fastest responses, basic accuracy",
      size: "1.3GB",
    },
    {
      id: "llama3.2:3b",
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        showModelDropdown &&
        !event.target.closest(".model-dropdown-container")
      ) {
    ...
    try {
      if (ollamaStatus) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            type: "bot",
            content:
              "Switched to Basic Mode. Advanced AI features are now disabled.",
            timestamp: new Date().toISOString(),
          },
        ]);
      } else {
        // Turn on Advanced Mode
        const response = await api.post("/chatbot/toggle-ollama", {
          action: "start",
        });
        if (response.data.success) {
          ...
          setMessages((prev) => [
            ...prev,
            {
              ...
              type: "bot",
              content:
                "Switched to Advanced Mode! You now have access to AI-powered conversations.",
              timestamp: new Date().toISOString(),
            },
          ]);
        } else {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          type: "bot",
          content:
            "Failed to toggle mode. Please ensure Ollama is installed on your system.",
          timestamp: new Date().toISOString(),
          isError: true,
        },
      ]);
  ...
  const handleModelChange = async (modelId) => {
    ...
    try {
      const response = await api.post("/chatbot/check-model", {
        model: modelId,
      });
  ...
  const handleInstallModel = async () => {
    ...
    try {
      const response = await api.post("/chatbot/install-model", {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: isOllamaOffline
            ? "Advanced chatbot is currently offline. I can still help with basic financial queries about your spending, budgets, and debts!"
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
                  This model is not installed on your system. Would you like to
                  download it now?
                </p>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-3 text-center">
                  Installation may take a few minutes depending on your
                  connection
                </p>
              </div>
              <button
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex items-center h-6 w-12 rounded-full transition-colors ${
                        ollamaStatus ? "bg-green-500" : "bg-gray-400"
                      } disabled:opacity-50`}
                    ...
                    >
                      <span
                        className={`inline-block h-5 w-5 bg-white rounded-full transform transition-transform ${
                          ollamaStatus ? "translate-x-6" : "translate-x-1"
                        }`}
                      ></span>
                    </button>
                  </div>
                ...
                <button
                  ...
                  className={`w-full py-2.5 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl ${
                    ollamaStatus
              ...
              >
                ...
                >
                  {isTogglingMode ? (
                    ...
                    <Power
                      className={`h-2 w-2 ${
                        ollamaStatus ? "text-green-600" : "text-gray-400"
                      }`}
                    />
                  )}
                </span>
