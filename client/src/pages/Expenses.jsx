...
const Expenses = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      if (editingExpense) return api.put(`/expenses/${editingExpense._id}`, data);
    ...
    <div className="space-y-4 md:space-y-6 p-4 md:p-0">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-200 mt-1">Track and manage your spending</p>
              ...
              {expensesData?.expenses?.map(expense => (
                <tr key={expense._id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-150">
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {expense.tags?.length > 0 && <div className="text-sm text-gray-500 dark:text-gray-400">{expense.tags.join(', ')}</div>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-red-600 dark:text-red-400">-{formatCurrency(expense.amount)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{formatDate(expense.date)}</td>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{editingExpense ? 'Edit Expense' : 'Add New Expense'}</h2>
                <button onClick={handleCloseModal} className="text-gray-400 dark:text-gray-300 hover:text-gray-600 dark:hover:text-gray-100">
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Account <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value })} className="input dark:bg-gray-800 dark:text-gray-100 dark:border-gray-700" placeholder="Store name, city, etc." />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" required value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="input dark:bg-gray-800 dark:text-gray-100 dark:border-gray-700" placeholder="What did you spend on?" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tags (comma-separated)</label>
                  <input type="text" value={formData.tags} onChange={e => setFormData({ ...formData, tags: e.target.value })} className="input dark:bg-gray-800 dark:text-gray-100 dark:border-gray-700" placeholder="groceries, food, lunch" />
                </div>
