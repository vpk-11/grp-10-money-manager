...
const Debts = () => {
  ...
  const handleEdit = (debt) => {
    ...
    setFormData({
      ...
      startDate: debt.startDate
        ? new Date(debt.startDate).toISOString().split("T")[0]
        : "",
      lender: debt.lender || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.principal || parseFloat(formData.principal) <= 0)
      return toast.error("Original amount must be > 0");
  ...
  const visibleDebts =
    debts?.filter((d) => d.status === "active" || d.status === "paid_off") ||
    [];
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="card bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-l-4 border-blue-500 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            ...
            <p className="text-3xl font-bold text-blue-600 dark:text-blue-400 mt-2">
              {formatCurrency(analytics.totalMonthlyPayment)}
            </p>
          </div>
      ...
      <div className="space-y-4">
        {visibleDebts.length === 0 ? (
          ...
          visibleDebts.map((debt) => {
            const isPaidOff =
              debt.currentBalance === 0 || debt.status === "paid_off";
            const daysUntilDue = isPaidOff
              ? null
              : calculateDaysUntilDue(debt.dueDate);
            ...
            return (
              <div
                ...
                className={`card transition-all duration-300 hover:shadow-xl ${
                  isPaidOff
                    ? "border-2 border-emerald-300 dark:border-emerald-600 bg-gradient-to-br from-emerald-50/50 to-emerald-100/50 dark:from-emerald-900/20 dark:to-emerald-800/30"
                    : "hover:-translate-y-1"
                }`}
              >
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {isPaidOff && (
                            <CheckCircle2 className="h-7 w-7 text-emerald-600 dark:text-emerald-400 animate-pulse" />
                          )}
                        </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900 rounded-lg"
                        >
                          <Edit className="h-4 w-4 text-gray-500 dark:text-gray-200" />
                        </button>
                        <button
                          onClick={() => deleteMutation.mutate(debt._id)}
                  ...
                  {!isPaidOff && debt.status === "active" && (
                    <div className="lg:w-64">
                      <div
                        className={`p-6 rounded-xl text-center shadow-lg border-2 ${
                          isUrgent
                            ? "bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-800/30 border-red-300 dark:border-red-700 animate-pulse"
