...
const Budgets = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => {
      ...
      if (editingBudget) return api.put(`/budgets/${editingBudget._id}`, payload);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.categoryId || !formData.amount || parseFloat(formData.amount) <= 0) return;
  ...
  const getStatusColor = (pct) => {
    if (pct >= 100) return 'text-red-600 bg-red-50 dark:bg-red-900 dark:text-red-400';
    if (pct >= 80) return 'text-orange-600 bg-orange-50 dark:bg-orange-900 dark:text-orange-400';
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Set limits and stay on track</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {budgets?.length === 0 ? (
          <div className="col-span-full text-center py-16 text-gray-700 dark:text-gray-300">
          ...
          budgets.map((budget) => {
            ...
            return (
              <div
                key={budget._id}
                className={`card p-6 bg-white dark:bg-gray-800 shadow-md dark:shadow-none ${over ? 'ring-2 ring-red-500' : ''}`}
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(budget)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                      <Edit className="h-4 w-4 text-gray-700 dark:text-gray-200" />
                    </button>
                    <button onClick={() => deleteMutation.mutate(budget._id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-900 rounded">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-end mb-2">
                      <span className={`text-3xl font-bold ${over ? 'text-red-600' : 'text-gray-900 dark:text-gray-100'}`}>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      ...
                      <p className={`font-bold text-lg ${over ? 'text-red-600' : 'text-gray-900 dark:text-gray-100'}`}>
                        {pct.toFixed(0)}%
                      </p>
                    </div>
                    <div>
                  ...
                  {budget.notes && (
                    <p className="text-sm text-gray-600 dark:text-gray-400 italic border-t pt-3">{budget.notes}</p>
                  )}
                </div>
              </div>
            );
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="input pl-10 dark:bg-gray-700 dark:text-gray-100"
                    placeholder="500.00"
                    step="0.01"
                    min="0"
                    required
